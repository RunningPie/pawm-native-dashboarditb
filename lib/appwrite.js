...
export const getAllContent = async (contentType) => {
    ...
    try {
        ...
        // console.log("All contents documents queried: ", allContents.documents);
...
export async function  updateDocument (databaseId, collectionId, documentId, data, permissions = []) {
  try {
...
export async function handleUpdateScore (documentId, score) {
  const databaseId = AWConfig.databaseId; // Replace with your database ID
  const collectionId = AWConfig.testsCollectionID; // Replace with your collection ID
  const dataToUpdate = {
    progress: score, // Example data to update
  };

  try {
    const updatedDocument = await updateDocument(databaseId, collectionId, documentId, dataToUpdate);
